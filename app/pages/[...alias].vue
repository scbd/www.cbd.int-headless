      ...
      <div class="protocol-subnavigation accent-cbd">
        <nav class="navbar container-xxl">
          <ul class="nav">
            ...
            <status v-if="menuPending || menuError" :error="menuError" />
            <template v-else>
              <navigation-top-menu
                v-if="megaMenu"
                :menus="megaMenu"
...
const { data: page, error, pending } = getContent(route.path)
...
const menuCode = computed(() => page.value?.menu ?? '')
const menuOptions = computed(() => ({ url: route.path, depth: 1 }))

const {
  data: menu,
  error: menuError,
  pending: menuPending
} = getMenu(menuCode, menuOptions)
...
const verticalMenu = computed(() =>
  menuRoot.value?.children?.find((i) => i.activeBranch)?.children?.find((i) => i.activeBranch)
)

const breadcrumbMenu = computed(() => {
  if (page.value?.alias && menuRoot.value) return buildPath(menuRoot.value)
  return undefined
})
