...
const themenu = props.submenu ? await getMenu(props.submenu) : props.menu;

const level2MenuColumnCount = computed(() => {
  const classes: string[] = [];
  const styles: { [key: string]: string | number } = {};

  if (themenu?.length && themenu?.length < 5) {
    styles['--level2-column-count'] = themenu.length;
  }
